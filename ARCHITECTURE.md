...
Общий nginx на 80/443 трогать нельзя, пока человек явно не попросил повесить пилот на домен. До тех пор сайт только на `127.0.0.1:3010`.
...
Одна лента на пару менеджер + клиент.
...
| `subject` | text | тема последнего/первого письма, для списка |
...
`standard` — один сотрудник. `twin` — два (две кровати / двухместный). Если едут двое, агент предпочитает один `twin` или два `standard` — что дешевле и есть в наличии на все ночи.
...
3. На каждое новое:
   ...
   - найти/создать `Client` и `Conversation`;
...
**Вход:** текст ответа, тема, последние 4–6 сообщений ленты (для тона, не для оценки клиента).
...
`showHint = true`, если хотя бы одно:

- любой критерий ≤ 3;
- `overall` < 4.0;
- в `issues` есть конкретная ошибка, которую менеджер может исправить в следующем письме.

Если всё 4–5 и замечаний нет — молчим. Иначе лента зашумится на демо.
...
Вход: вся лента (для пилота письма короткие) или последние ~8000 символов.
...
- Города прогнать через `City.aliases` **кодом после модели**: модель может вернуть «Питер» — резолвер превращает в `LED`. Если не нашли — `unresolved` и карточка «не понял город, уточните».
- `people` по умолчанию 1.
...
- Если дат/городов нет — не выдумывать; вернуть `confidence<0.5` и спросить уточнение без SQL.
...
Для каждого отеля города назначения:
  ...
  roomType = people>=2 ? twin : standard
  если на ВСЕ ночи roomsLeft>=1 (для twin) или нужное число standard
  cost = сумма pricePerNight * комнаты
  иначе отель отбрасываем
...
Низ: textarea, «Отправить», «Подобрать решение». Пока job в `processing` — спиннер на кнопке / тонкая плашка «ИИ подбирает варианты…».
...
- Графики Recharts: средняя оценка по менеджерам (bar); динамика `overall` по дням (line, если точек мало — всё равно рисуем); число подсказок (bar).
...
Это сознательная дыра безопасности. Стенд не выставлять в открытый интернет без дополнительной защиты (уже есть риск: порты лучше слушать на `127.0.0.1` и открывать через SSH-туннель / существующий nginx с ограничением). В `docker-compose` для пилота допустимо `127.0.0.1:3010:3010`, как у mapvideo.
...
| Число дней | 136 |
| Городов | 22 |
| Отелей | 3 или 4 на город → **80** |
| Типы номеров | `standard`, `twin` |
| Строк `hotel_availability` | 80 × 136 × 2 ≈ **21 760** |
...
На каждый город 3 или 4 отеля (чётный hash → 4, иначе 3). Имена выдумать по шаблону, без чужих торговых марок: «Гостиница Невский 38», «Отель Река», «Аврора Plaza», «Сибирский двор», «Тихий центр» + имя города.
...
| GET | `/api/inbox/snapshot` | manager/chief | ленты + дельта сообщений |
...
| GET | `/api/admin/stats` | chief | агрегаты отдела и менеджеров |
