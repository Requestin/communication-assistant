...
Этот файл — единственный источник правды по тому, *что* мы строим и *как*. 
...
| Сборка          | Docker Compose, 4 сервиса                                           | Одной командой поднять стенд                                            |
...
У нас и так Docker, нужны нормальные выборки по рейсам/отелям и агрегаты для админки. Postgres в контейнере почти не дороже и ближе к «как это выглядело бы дальше».
...
| `deletedAt`     | timestamptz null                   | скрыта в ленте; строки для статистики остаются |
...
| `deletedAt`      | timestamptz null | скрыто в ленте; оценка и счётчики остаются |
...
| `id`             | uuid PK                          |                                                    |
| `conversationId` | uuid FK                          |                                                    |
| `messageId`      | uuid null FK                     | для подсказки качества — какое исходящее оценивали |
...
| `deletedAt`      | timestamptz null                 | скрыта в ленте; подборы на консоли всё равно считаются |
| `createdAt`      | timestamptz                      |                                                    |
...
После забора новых UID — сверка удаления (§8.7): входящие, которых нет в `INBOX`, и исходящие, которых нет в «Отправленных», помечаются `deletedAt`. Строки не стираем.
...
### 8.7. Двустороннее удаление (Корзина Gmail)

Письма и диалоги в ленте совпадают с ящиком менеджера. Удаление **мягкое**: `deletedAt`, строки остаются. Консоль главного считает все строки, в том числе скрытые: средние оценки, доля подсказок, число ответов, графики, число подборов не откатываются. `Client` не удаляем.

**Gmail → сервис** (тот же опрос IMAP ~15 с):

1. Входящие: UID нет в `INBOX` (письмо в Корзине или снято с входящих) → `Message.deletedAt`.
2. Исходящие с сайта: нет в папке `\Sent` по `smtpMessageId` → `Message.deletedAt`.
3. Живых писем в ленте не осталось → `Conversation.deletedAt` и скрыть живые `AiNote` этой ленты.
4. Письмо вернулось во Входящие / Отправленные (тот же UID / Message-ID) → снять `deletedAt` с письма и ленты. Карточки ИИ сами не оживают.
5. Новое входящее в ту же тему оживляет скрытую ленту (`deletedAt=null` на `Conversation`).

Папки `\Trash` и `\Sent` искать по IMAP special-use, не по русскому имени.

**Сервис → Gmail** (сразу в API, не через очередь заданий):

- Удалить диалог: `MOVE` всех входящих этой темы из `INBOX` и исходящих из `\Sent` в `\Trash`.
- Удалить исходящее: то же для одного письма в `\Sent`.
- Карточка ИИ в Gmail не живёт — только `AiNote.deletedAt`.
- Письма в ящике уже нет — всё равно скрываем у себя.
- Сеть или логин Gmail упали — **не** скрываем, API 502 «Не удалось удалить в почте».

**Кто может в UI**

- Менеджер: крестик на своём диалоге слева; в ленте — только свои исходящие и карточки ИИ. Входящее клиента из чата не удаляется.
- Главный: кнопок нет, API 403.
...
- нет приветствия, **только если код поставил «проверять приветствие»** (см. ниже);
- нет прощания в письме длиннее двух строк (для совсем короткого подтверждения не штрафовать жёстко) — правило прощания на этапе 9.9 не меняем;
- обещание «всё забронировал», которого менеджер не мог выполнить с сайта.

Приветствие **ожидается**, только если верно хотя бы одно (считает **код**, не модель):

1. это первый исходящий менеджера в ленте (ответ на первое письмо клиента);
2. с предыдущего сообщения любой стороны прошло **больше 8 часов**;
3. последнее входящее клиента перед этим ответом само содержит приветствие («здравствуйте», «добрый день/утро/вечер», «привет»).

Иначе отсутствие «здравствуйте» **не** снижает `businessStyle` и не попадает в `issues` / `hint`. В активной переписке здороваться в каждом письме не нужно.
...
У менеджера на карточке диалога слева крестик (не внутри кнопки открытия ленты): удаляет всю тему в сервисе и кладёт письма в Корзину Gmail. Перед этим — короткое подтверждение на русском. У главного крестика нет.

Справа лента:

- входящие — слева/нейтральные; менеджер их из чата не удаляет;
- исходящие — справа; у менеджера крестик на своём письме;
- `AiNote` — на всю ширину, фон янтарь, бейдж «Только для сотрудников»; у менеджера крестик на карточке.
...
- скрытые `deletedAt` в эти формулы **не** входят как исключение: консоль не уменьшает уже набранное после удаления в ленте.
...
Это сознательная дыра безопасности. Человек выставил стенд на **[https://assistant.gyhyry.com](https://assistant.gyhyry.com)** (nginx → `127.0.0.1:3010`). Порт 3010 по-прежнему не слушает внешний интерфейс. Входа с паролем нет — это учебный стенд. В `docker-compose` для пилота допустимо `127.0.0.1:3010:3010`, как у mapvideo.
...
Цены и расписание **выдуманные**. В UI и в тексте клиенту эту оговорку не пишем.
...
**Готово когда:** деловое «Сейчас подберём…» молчит; «Привет Ща» - карточка.
...
- Публичный вход: **[https://assistant.gyhyry.com](https://assistant.gyhyry.com)**.
...
Файл этапа: [docs/stages/09-8-copy-cleanup.md](docs/stages/09-8-copy-cleanup.md)

- Нет «учебные данные, не оферта» в карточке, вставке в письмо и на входе.
- Нет подписей «ответ уйдёт на почту клиента» и «Слева диалоги по темам писем…».

**Готово когда:** в живом UI этих фраз нет.

### Этап 9.9. Приветствие только в начале и после паузы

Файл этапа: [docs/stages/09-9-agent-hello-fix.md](docs/stages/09-9-agent-hello-fix.md)

- Приветствие в оценке только: первый ответ клиенту, простой > 8 часов, или клиент сам поздоровался в последнем письме.
- В активной переписке отсутствие «здравствуйте» оценку не снижает.

**Готово когда:** середина ленты без приветствия молчит; первое письмо и ответ после паузы по-прежнему могут штрафовать.

### Этап 10. Двустороннее удаление с Gmail

Файл этапа: [docs/stages/10-mail-delete-sync.md](docs/stages/10-mail-delete-sync.md)

- Мягкое `deletedAt` у ленты, письма и карточки ИИ.
- Опрос IMAP прячет то, чего нет во Входящих / Отправленных; удаление в сервисе кладёт письмо в Корзину Gmail.
- Менеджер удаляет диалог, свои исходящие и карточки ИИ. Главный — нет. Консоль не откатывает цифры.

**Готово когда:** письмо, удалённое в Gmail, пропадает в ленте за ≤20 с; удаление диалога в сервисе кладёт письма в Корзину; цифры главного не уменьшаются.
...
Открыть `http://127.0.0.1:3010` или **[https://assistant.gyhyry.com](https://assistant.gyhyry.com)**.
...
| DELETE | `/api/conversations/:id`         | владелец       | скрыть диалог и унести письма в Корзину Gmail                                               |
| DELETE | `/api/conversations/:id/messages/:messageId` | владелец | скрыть своё исходящее и унести его в Корзину; входящее — 403                    |
| DELETE | `/api/conversations/:id/notes/:noteId` | владелец | скрыть карточку ИИ; в Gmail не ходим                                                |
