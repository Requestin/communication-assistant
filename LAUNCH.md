# Запуск пилота на новом Linux-сервере

Пошаговая инструкция, как поднять **Помощник в коммуникации с клиентами** на пустом сервере. Это не установщик коммерческого продукта: один хост, живая почта Gmail, локальная модель.

Сценарий показа для бизнеса — в [README.md](README.md). Устройство сервиса — в [ARCHITECTURE.md](ARCHITECTURE.md). Сюда секреты и пароли почты **не** копировать.
...
Если на машине уже крутится другой стек (в том числе mapvideo): не останавливайте чужие контейнеры, не занимайте 3000 / 3001 / 3002 / 5432 / 5000, не делайте `docker compose down` вне каталога этого репозитория.
...
| GPU | NVIDIA, VRAM с запасом под файл ~22 ГБ и контекст 8192 (карта класса 24–32 ГБ). Запуск только на CPU здесь не описан |
| Диск | свободно **не меньше 80 ГБ** |
| Сеть | исходящий HTTPS (GitHub, Hugging Face, Docker Hub, Gmail) |
| Почта | три Gmail менеджеров и пароли приложений (не обычный пароль ящика) |
...
Соберите его туда (нужен свежий llama.cpp с поддержкой Qwen3.6; на исходном стенде была сборка порядка b9206):
...
Проверка размера (ожидается порядка 22 ГБ, не килобайтный указатель):
