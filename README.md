...
Сервис имитирует рабочее место менеджера, который оформляет командировки сотрудникам клиентских компаний. Клиент пишет сотруднику на обычную почту — сотрудник ведет переписку внутри сервиса и имеет дополнительный функционал в виде подсказок по тону ответа и подбора вариантов «перелёт + отель» исходя из условий клиента. Человек всегда сам решает, что отправить.
...
- **Удобный UI вместо почтового ящика.** Все переписки с клиентами в одном месте. Ответ сотрудника уходит клиенту обычным письмом.
...
У каждого менеджера своя лента. Руководитель клиентам не пишет: смотрит статистику в консоли и при необходимости открывает любую переписку.

Цены и наличие мест в пилоте **учебные**. Это не реальные предложения перевозчиков и брони отелей.
...
4. Ответьте нарочно небрежно: «ок ща гляну камандировку». В ленте появится жёлтая подсказка для сотрудника. Убедитесь, что у клиента в ящике её нет.
...
2. В консоли — цифры по отделу и по каждому менеджеру: как отвечают, где проседает стиль, как меняется оценка.
...
Смысл: агент не станет выдумывать несуществующую бронь, а сообщит менеджеру. Лучше честный отказ, чем красивая ошибка.
...
| Подбор из демо-справочника по последним условиям клиента                | Больше городов, больше сотрудников, привычные вложения к письмам             |
| Консоль руководителя со статистикой по качеству переписки               | Регламенты и сроки ответа, сводка не только по стилю, но и по скорости       |
